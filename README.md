...
Dependencies are managed with [uv](https://docs.astral.sh/uv/) (`pyproject.toml`, `uv.lock`). Python 3.14 or higher is recommended.
...
Build and run from the repository root. The image uses `uv.lock` for reproducible installs. Do not bake secrets into the image—pass `OPENAI_API_KEY` at runtime.
...
├── preview_helpers.py            # Preview/export helpers for UI
├── tests/
└── README.md
```
